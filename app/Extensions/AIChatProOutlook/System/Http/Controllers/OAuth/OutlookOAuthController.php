...
namespace App\Extensions\AIChatProOutlook\System\Http\Controllers\OAuth;
...
{
    private const CONNECTOR_TYPE = 'outlook';
    ...
    public function redirect(Request $request): RedirectResponse
    {
        ...
        if ($request->boolean('popup')) {
            session(['ai_chat_pro_outlook_oauth_popup' => true]);
        }
    ...
    {
        $isPopup = session()->pull('ai_chat_pro_outlook_oauth_popup', false);
        ...
        if ($isPopup) {
            return $this->popupClose(true, 'Outlook account connected successfully.', ['key' => self::CONNECTOR_TYPE]);
        }

        $accessUrl = route('dashboard.user.ai-chat-pro.connectors.access.show', $connector);

        return redirect()->to($accessUrl)->with(['type' => 'success', 'message' => __('Outlook account connected successfully.')]);
    }
