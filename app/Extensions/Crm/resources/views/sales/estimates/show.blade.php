...
@section('title', $item->estimate_number)
...
@section('titlebar_actions')
	<div class="flex gap-2">
		<x-button
			...
			href="{{ route('dashboard.user.sales.estimates.index') }}"
		>
			<x-tabler-arrow-left class="size-4" />
			{{ __('Back to Estimates') }}
		</x-button>
		<x-button
			variant="ghost-shadow"
			hover-variant="danger"
			id="crm_delete_estimate_btn"
		>
...
@section('content')
	<div
		class="py-10"
		x-data="estimateItems()"
	>
		<div class="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
			<x-card class:body="flex flex-col gap-2 p-5">
				<div class="flex items-center justify-between">
					<h3 class="text-lg font-bold">{{ $item->estimate_number }}</h3>
					@switch($item->status)
						@case('accepted')
...
@push('script')
	<script>
		document.getElementById('crm_delete_estimate_btn')?.addEventListener('click', function() {
			if (!confirm('{{ __("Are you sure you want to delete this estimate?") }}')) return;
			...
			$.ajax({
				type: 'GET',
				url: '{{ route("dashboard.user.sales.estimates.delete", $item->id) }}',
				success: function(data) {
					toastr.success(data.message || '{{ __("Estimate deleted successfully.") }}');
					if (data.redirect) {
						setTimeout(function() { window.location.href = data.redirect; }, 300);
					} else {
						setTimeout(function() { window.location.href = '{{ route("dashboard.user.sales.estimates.index") }}'; }, 300);
					}
				},
				error: function(data) {
		...
		@php
			$estimateItems = $item->items->map(fn($i) => [
		...
		function estimateItems() {
			return {
				...
				saveItems() {
					...
					$.ajax({
						type: 'POST',
						url: '{{ route("dashboard.user.sales.estimates.saveItems", $item->id) }}',
