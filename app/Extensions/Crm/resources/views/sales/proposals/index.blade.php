...
@section('titlebar_subtitle', __('Create proposals for your prospects and clients.'))

@section('titlebar_actions')
	<x-modal title="{{ __('Create Proposal') }}">
		<x-slot:trigger>
		...
		<x-slot:modal>
			<form
				...
				onsubmit="return salesSubmitForm(event, '{{ route('dashboard.user.sales.proposals.store') }}')"
			>
				@csrf
				<x-forms.input
					size="lg"
				...
				<x-forms.input size="lg" type="select" label="{{ __('Deal') }}" name="crm_deal_id">
					...
					<div class="flex flex-col">
						@foreach ($sort_buttons as $button)
							<a
								...
								href="{{ route('dashboard.user.sales.proposals.index', ['filter' => $filter, 'sort' => $button['sort'], 'sort_dir' => ($sort === $button['sort'] && $sortDir === 'asc') ? 'desc' : 'asc']) }}"
							>
								{{ $button['label'] }}
								@if ($sort === $button['sort'])
			...
			<div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-heading-foreground max-sm:gap-3">
				@foreach ($filter_buttons as $button)
					<x-button
						...
						tag="a"
						href="{{ route('dashboard.user.sales.proposals.index', ['filter' => $button['filter'], 'sort' => $sort, 'sort_dir' => $sortDir]) }}"
						variant="ghost"
					>
						{{ $button['label'] }}
					</x-button>
		...
		<x-table>
			...
			<x-slot:body>
				@forelse ($list as $entry)
					<tr>
						<td>
							<a class="font-medium text-primary hover:underline" href="{{ route('dashboard.user.sales.proposals.show', $entry->id) }}">{{ $entry->proposal_number }}</a>
						</td>
						...
						<td class="whitespace-nowrap text-end">
							<x-modal class="inline-flex" title="{{ __('Edit Proposal') }} - {{ $entry->proposal_number }}">
								<x-slot:trigger class="size-9" size="none" variant="ghost-shadow" title="{{ __('Edit') }}">
									<x-tabler-pencil class="size-4" />
								</x-slot:trigger>

								<x-slot:modal>
									<form class="flex flex-col gap-5" onsubmit="return salesSubmitForm(event, '{{ route('dashboard.user.sales.proposals.update', $entry->id) }}')">
										@csrf
										@method('PUT')
										...
										<x-forms.input size="lg" type="select" label="{{ __('Deal') }}" name="crm_deal_id">
											<option value="">{{ __('Select a deal') }}</option>
											@foreach ($deals as $deal)
												<option value="{{ $deal->id }}" @selected($entry->crm_deal_id == $deal->id)>{{ $deal->title }}</option>
											@endforeach
										</x-forms.input>
										<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
							...
							<x-button class="size-9" variant="ghost-shadow" hover-variant="danger" size="none" onclick="return confirm('{{ __('Are you sure you want to delete this proposal?') }}')" href="{{ route('dashboard.user.sales.proposals.delete', $entry->id) }}" title="{{ __('Delete') }}">
				...
				@empty
					<tr>
						<td class="text-center text-foreground/60" colspan="7">
							{{ __('No proposals yet. Create your first one!') }}
						</td>
					</tr>
				@endforelse
			</x-slot:body>
