<x-table
    class="relative"
    id="social-media-agent-posts-list"
...
>
    ...
    <x-slot:body>
        @include('social-media-agent::components.posts.list.post-items', ['items' => $posts])
    </x-slot:body>
    ...
    >
        @if ($posts->hasMorePages())
            <tr>
                <td colspan="5">
                    <div class="social-media-agent-posts-list-load-more-wrap">
                        <a
                            class="social-media-agent-posts-list-load-more group inline-flex w-full items-center justify-center gap-2 text-xs font-medium"
                            href="{{ route('dashboard.user.social-media.agent.post-items', ['page' => 2, 'post_style' => 'list']) }}"
