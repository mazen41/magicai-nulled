...
>
    ...
    <p class="mb-3 flex items-center justify-center gap-1 font-medium text-blue-500">
                    ...
                    >
                        ...
                        >
                            ...
                            <input
                                ...
                                {{-- x-effect="$nextTick(() => picker.setColor(customerTagModal.form.tag_color))" --}}
                            />
                        </div>
                        <x-forms.input
    ...
    <div class="rounded-lg border px-4 xl:mx-4">
        ...
        <p class="mb-0 flex items-center justify-between gap-1 border-b py-4 text-foreground/60">
            ...
            @if (\App\Helpers\Classes\Helper::appIsDemo())
                <span class="max-w-[65%] overflow-hidden text-ellipsis whitespace-nowrap text-end capitalize text-foreground">{{ trans('1 hour ago') }}</span>
            @else
                <span
                    class="max-w-[65%] overflow-hidden text-ellipsis whitespace-nowrap text-end capitalize text-foreground"
    ...
    <p class="-mx-4 mb-5 mt-5 flex items-center justify-center gap-10">
        <span class="inline-block h-px grow bg-current opacity-5"></span>
        {{ __('Visited Pages') }}
        <span class="inline-block h-px grow bg-current opacity-5"></span>
    </p>
    ...
    <div class="rounded-lg border px-4 xl:mx-4">
        <template x-if="activeChat?.visited_pages?.length">
            <div>
                <template
                    x-for="(page, index) in activeChat.visited_pages"
                    :key="index"
                >
                    <p
                        class="mb-0 flex items-center justify-between gap-1 py-4 text-foreground/60"
                        :class="{ 'border-b': index < activeChat.visited_pages.length - 1 }"
                    >
                        <a
                            class="group flex max-w-[65%] items-center gap-1 overflow-hidden text-ellipsis whitespace-nowrap text-foreground/60 no-underline hover:text-foreground"
                            :href="page.page_url"
                            :title="page.page_url"
                            target="_blank"
                        ...
                        >
                            <span
                                ...
                                x-text="page.page_title || page.page_url"
                            ></span>
                            <x-tabler-external-link class="size-3.5 shrink-0 opacity-0 transition-opacity group-hover:opacity-100" />
                        </a>
                        <span
                            class="shrink-0 text-end text-foreground"
                            x-text="(() => {
                                if (!page.duration && page.duration !== 0) return '---';
                                const s = page.duration;
                                if (s < 60) return s + 's';
        ...
        <p
            class="mb-0 py-4 text-center text-foreground/40"
            x-show="!activeChat?.visited_pages?.length"
        >
            {{ __('No visited pages yet') }}
        </p>
    </div>
