<div class="lqd-ext-chatbot-history-head sticky top-0 z-1 h-14 w-full shrink-0 border-b bg-background px-4 lg:h-[--header-height] lg:border-b-0 lg:bg-transparent">
    ...
    >
        <span
            ...
            x-text="activeConv?.workflow_name || activeConv?.channel?.name || activeConv?.sender_id || '{{ __('Select a conversation') }}'"
        ></span>
        ...
        <div class="ms-auto hidden grow items-center justify-end gap-2 lg:flex">
            ...
            <x-button
                ...
                x-show="activeConv"
                @click.prevent="pinConversation(activeConv.id)"
            >
                <x-tabler-pin
                    class="size-4"
                    x-show="!activeConv?.pinned"
                />
                <x-tabler-pinned
                    ...
                    <ul>
                        <li class="border-b last:border-b-0">
                            <x-button
                                ...
                                href="#"
                                variant="link"
                                @click.prevent="deleteConversation(activeConv.id)"
                            >
                                {{ __('Delete') }}
                            </x-button>
                        </li>
                        <li class="border-b last:border-b-0">
                            ...
                            <div
                                ...
                                x-collapse
                            >
                                @foreach ([['format' => 'txt', 'label' => __('Download as Text'), 'icon' => 'tabler-file-type-txt'], ['format' => 'csv', 'label' => __('Download as CSV'), 'icon' => 'tabler-file-type-csv'], ['format' => 'pdf', 'label' => __('Download as PDF'), 'icon' => 'tabler-file-type-pdf'], ['format' => 'json', 'label' => __('Download as JSON'), 'icon' => 'tabler-json']] as $exportFormat)
                                    <a
                                        class="flex items-center gap-2 py-2 text-xs font-medium hover:underline"
                                        :href="`${threadBaseUrl}/${activeConv?.id}/export?format={{ $exportFormat['format'] }}`"
                                    >
                                        <x-dynamic-component
                                            class="size-4"
        ...
        <div class="ms-auto flex items-center gap-0.5 lg:hidden">
            ...
            <x-button
                ...
                variant="none"
                size="none"
                ::class="{ 'active': mobile.contactInfoVisible && contactInfo.activeTab === 'logs' }"
                @click.prevent="mobile.filtersVisible = false; mobile.contactInfoVisible = mobile.contactInfoVisible && contactInfo.activeTab === 'logs' ? false : true; contactInfo.activeTab = 'logs';"
                aria-label="{{ __('Logs') }}"
            >
                <svg
                    width="21"
    ...
    >
        <x-forms.input
            ...
            @input.debounce.200ms="searchMessages($el.value)"
        ...
        <x-button
            ...
            @click.prevent="messagesSearchFormVisible = false; searchMessages('')"
            size="none"
        >
