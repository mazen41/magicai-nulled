...
namespace App\Extensions\AIChatProGoogleDrive\System\OAuth;
...
class GoogleDriveOAuth
{
    ...
    {
        $this->config = $config ?? config('ai-chat-pro-google-drive', []);

        foreach (['client_id', 'client_secret', 'redirect_uri'] as $key) {
            $value = setting('ai_chat_pro_google_drive_' . $key);
            if (! empty($value)) {
    ...
    {
        return 'ai_chat_pro_google_drive.oauth.state.' . $state;
    }

    private function redirectUri(): string
    {
        return $this->config['redirect_uri']
            ?? secure_url(route('dashboard.user.ai-chat-pro.connectors.google-drive.callback', [], false));
    }
}
