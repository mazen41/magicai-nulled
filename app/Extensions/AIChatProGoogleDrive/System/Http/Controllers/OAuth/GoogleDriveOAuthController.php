...
namespace App\Extensions\AIChatProGoogleDrive\System\Http\Controllers\OAuth;
...
{
    private const CONNECTOR_TYPE = 'google_drive';
    ...
    public function redirect(Request $request): RedirectResponse
    {
        ...
        if ($request->boolean('popup')) {
            session(['ai_chat_pro_google_drive_oauth_popup' => true]);
        }
    ...
    {
        $isPopup = session()->pull('ai_chat_pro_google_drive_oauth_popup', false);
        ...
        if ($isPopup) {
            return $this->popupClose(true, 'Google Drive account connected successfully.', ['key' => self::CONNECTOR_TYPE]);
        }

        $accessUrl = route('dashboard.user.ai-chat-pro.connectors.access.show', $connector);

        return redirect()->to($accessUrl)->with(['type' => 'success', 'message' => __('Google Drive account connected successfully.')]);
    }
